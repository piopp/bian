...
def cancel_cm_order():
    ...
    try:
        ...
        log_info(f"使用币本位合约取消订单API: {endpoint}, 参数: {params}")
        ...
        if response.get('success'):
            ...
            error_msg = truncate_message(response.get('error', '取消币本位合约订单失败'))
            log_error(f"取消子账号 {email} 币本位合约订单失败: {error_msg}")
            return jsonify({
                "success": False,
                "error": f"取消币本位合约订单失败: {error_msg}"
            }), 400
            
    except Exception as e:
        error_msg = truncate_message(str(e))
        log_exception(f"取消币本位合约订单异常: {error_msg}")
        return jsonify({
            "success": False,
            "error": f"取消币本位合约订单异常: {error_msg}"
        }), 500
...
def place_cm_order():
    ...
    try:
        ...
        # 使用币本位合约下单API - PAPI
        endpoint = "/papi/v1/cm/order"
        log_info(f"使用币本位合约下单API: {endpoint}, 参数: {params}")
        ...
        if response.get('success'):
            ...
            error_msg = truncate_message(response.get('error', '币本位合约下单失败'))
            log_error(f"子账号 {email} 币本位合约下单失败: {error_msg}")
            
            # 如果是持仓模式不匹配的错误，提供更详细的错误信息
            if "position side does not match" in error_msg:
                return jsonify({
                    "success": False,
                    "error": f"币本位合约下单失败: 持仓方向({params.get('positionSide')})与账户设置({('双向持仓' if dual_side_position else '单向持仓')})不匹配"
                }), 400
            
            return jsonify({
                "success": False,
                "error": f"币本位合约下单失败: {error_msg}"
            }), 400
            
    except Exception as e:
        error_msg = truncate_message(str(e))
        log_exception(f"币本位合约下单异常: {error_msg}")
        return jsonify({
            "success": False,
            "error": f"币本位合约下单异常: {error_msg}"
        }), 500


@subaccounts_bp.route('/cm/trades', methods=['POST'])
def get_cm_trades():
    """
    获取子账号的CM账户成交历史 - 使用PAPI接口
    ...
    try:
        ...
        for email in emails:
            try:
                ...
                log_info(f"使用币本位合约接口查询成交历史: {endpoint}, 参数: {params}")
                
                response = client._send_request('GET', endpoint, signed=True, params=params)
                
                if not response.get('success'):
                    error_msg = truncate_message(response.get('error', '获取CM账户成交历史失败'))
                    log_error(f"获取CM账户成交历史失败: {error_msg}")
                ...
                log_info(f"成功获取CM/币本位账户成交历史，条数: {len(trades)}")
                ...
                error_msg = truncate_message(str(e))
                log_exception(f"获取子账号 {email} 的CM账户成交历史异常: {error_msg}")
                results.append({
                    "email": email,
                    "success": False,
                    "error": f"获取CM账户成交历史异常: {error_msg}"
                })
        ...
        error_msg = truncate_message(str(e))
        log_exception(f"获取CM账户成交历史异常: {error_msg}")
        return jsonify({
            "success": False,
            "error": f"获取CM账户成交历史异常: {error_msg}"
        }), 500
...
@subaccounts_bp.route('/cm/orders', methods=['POST'])
def get_cm_orders():
    """
    获取子账号的币本位合约(CM)订单信息 - 使用PAPI接口
    ...
    try:
        ...
        for email in emails:
            try:
                ...
                # 获取未平仓订单 - 使用币本位合约API - PAPI
                log_info(f"获取币本位合约未平仓订单 (PAPI接口)")
                open_orders_endpoint = "/papi/v1/cm/openOrders"
                
                # 确保请求参数中包含合约标识
                open_params = params.copy()
                # 添加合约类型参数(虽然路径已经包含/cm/，但在记录日志时有用)
                open_params['contractType'] = 'CM'
                ...
                if symbol:
                    log_info(f"获取币本位合约历史订单 (PAPI接口)")
                    history_orders_endpoint = "/papi/v1/cm/allOrders"
                    
                    # 确保请求参数中包含合约标识
                    history_params = params.copy()
                    # 添加合约类型参数(虽然路径已经包含/cm/，但在记录日志时有用)
                    history_params['contractType'] = 'CM'
                ...
                error_msg = truncate_message(str(e))
                log_exception(f"获取子账号 {email} 的币本位合约订单异常: {error_msg}")
                results.append({
                    "email": email,
                    "success": False,
                    "error": f"获取币本位合约订单异常: {error_msg}"
                })
        ...
        error_msg = truncate_message(str(e))
        log_exception(f"获取币本位合约订单信息异常: {error_msg}")
        return jsonify({
            "success": False,
            "error": f"获取币本位合约订单信息异常: {error_msg}"
        }), 500


@subaccounts_bp.route('/cm/order', methods=['POST'])
def get_cm_order():
    """
    获取子账号的币本位合约(CM)当前交易对挂单信息 - 使用PAPI接口 - 只查询openOrder
    ...
    try:
        ...
        for email in emails:
            try:
                ...
                # 获取未平仓订单 - 使用币本位合约API - PAPI - 只使用openOrder接口
                log_info(f"获取币本位合约未平仓订单 (PAPI接口) 当前交易对: {symbol}")
                open_order_endpoint = "/papi/v1/cm/openOrder"
                ...
                error_msg = truncate_message(str(e))
                log_exception(f"获取子账号 {email} 当前交易对的币本位合约挂单异常: {error_msg}")
                results.append({
                    "email": email,
                    "success": False,
                    "error": f"获取币本位合约挂单异常: {error_msg}"
                })
        ...
        error_msg = truncate_message(str(e))
        log_exception(f"获取币本位合约挂单信息异常: {error_msg}")
        return jsonify({
            "success": False,
            "error": f"获取币本位合约挂单信息异常: {error_msg}"
        }), 500 