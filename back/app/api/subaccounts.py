...
def get_main_account_api_credentials(user_id=None):
    ...
    try:
        ...
        if active_api_key:
            ...
            default_api_secret = current_app.config.get('DEFAULT_API_SECRET', '')
...
def get_sub_account_api_credentials(email):
    ...
    try:
        ...
        if api_setting and api_setting.api_key and api_setting.api_secret:
            ...
            logger.warning(f"未在SubAccountAPISettings表中找到子账号API密钥 [email={email}]")
            return '', ''
    except Exception as e:
...
def get_subaccounts():
    ...
    if result.get('success') and 'data' in result:
        ...
        if 'subAccounts' in result['data']:
            ...
            logger.warning(f"API返回数据结构中没有subAccounts字段: {list(result['data'].keys())}")
    else:
...
def batch_create_subaccounts():
    ...
    for _ in range(count):
        ...
        # 生成随机字符串
        random_str = ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(random_length))
    ...
    # 统计成功和失败的数量
    success_count = sum(1 for r in results if r["result"]["success"] and "data" in r["result"] and "email" in r["result"]["data"])
...
def transfer_funds():
    ...
    logger.info(f"正在进行转账: 从 {from_email} 到 {to_email}, 资产: {asset}, 金额: {amount}")
    
    client = BinanceClient(api_key, api_secret)
    result = client.sub_account_transfer(from_email, to_email, asset, amount)
    
    if result.get('success'):
        logger.info(f"转账成功: 从 {from_email} 到 {to_email}, 资产: {asset}, 金额: {amount}")
    else:
        error_msg = result.get('error', '未知错误')
        ...
        if include_master:
            ...
            if auth_header and auth_header.startswith('Bearer '):
                ...
                try:
                    ...
                    # 获取主账号API设置
                    master_api = APIKey.query.filter_by(user_id=user_id, is_active=True).first()
                    if master_api:
                ...
                try:
                    ...
                    if settings and len(settings) > 0:
                        ...
                        if api_key and api_secret:
                            # 查找现有记录
                            existing = APIKey.query.filter_by(user_id=user_id).first()
    ...
    try:
        ...
        if response.get('success'):
            ...
            # 尝试获取币本位合约账户
                ...
                assets_data = futures_data.get('futureAccountResp', {}).get('assets', [])
                ...
                if backup_response.get('success'):
                    ...
                    for sub_account in sub_accounts:
                        if sub_account.get('email') == email:
                            # 提取该子账号的合约余额信息
                            total_balance = float(sub_account.get('totalWalletBalance', '0'))
            ...
            if len(valid_balances) == 0 and not asset:
                ...
                # 方法3: 使用账户快照API
                third_response = client.get_sub_account_snapshot(email, 'FUTURES', 7)
                    ...
                    if snapshots and len(snapshots) > 0:
                        ...
                        if 'assets' in snapshot_data:
                            ...
                            for asset_data in assets:
                                ...
                                asset_amount = float(asset_data.get('walletBalance', '0'))
...
def get_futures_positions():
    ...
    try:
        ...
        # 处理每个子账号
        for email in emails:
            logger.info(f"使用主账号查询子账号 {email} 的合约持仓")
            
            # 调用币安API获取持仓信息
            response = client.get_sub_account_futures_positions(email, symbol)
            ...
            if response.get('success'):
                ...
                for position in positions_data:
                    # 过滤掉空仓位 (positionAmt为0或positionAmount为0)
                    position_amt = float(position.get('positionAmt', position.get('positionAmount', '0')))
                        ...
                        try:
                            ...
                            if entry_price_float > 0 and position_amt != 0:
                                if position_amt > 0:  # 多头
                                    pnl_percentage = ((mark_price_float - entry_price_float) / entry_price_float) * 100
                                else:  # 空头
                                    pnl_percentage = ((entry_price_float - mark_price_float) / entry_price_float) * 100
                ...
                results.append({
                    "email": email,
                    "success": False,
                    "error": response.get('error', '获取持仓信息失败')
                })
...
def get_futures_orders():
    ...
    try:
        ...
        # 如果提供了userId，则使用主账户API查询；否则使用子账户自己的API查询
        if user_id:
            # 获取主账号API凭证
            api_key, api_secret = get_main_account_api_credentials(user_id)
        ...
        for email in emails:
            ...
            if user_id:
                # 使用主账号凭证创建的客户端
                logger.info(f"使用主账号API查询子账号 {email} 的合约订单")
                client = main_client
                response = client.get_sub_account_futures_orders(email, symbol, limit)
            else:
                # 从API设置表获取子账号API凭证
                logger.info(f"使用子账号 {email} 自身的API查询合约订单")
            ...
            if response.get('success'):
                ...
                results.append({
                    "email": email,
                    "success": False,
                    "error": response.get('error', '获取订单信息失败')
                })
        ...
        logger.exception(f"获取合约订单信息失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": f"获取合约订单信息失败: {str(e)}"
        }), 500
...
def get_futures_limit_orders():
    ...
    try:
        ...
        for email in emails:
            ...
            if user_id:
                # 使用主账号凭证创建的客户端
                logger.info(f"使用主账号API查询子账号 {email} 的合约挂单")
                client = main_client
                response = client.get_sub_account_futures_open_orders(email, symbol)
            else:
                # 从API设置表获取子账号API凭证
            ...
            if response.get('success'):
                ...
                results.append({
                    "email": email,
                    "success": False,
                    "error": response.get('error', '获取挂单信息失败')
                })
...
def diagnose_api_settings():
    ...
    """
    try:
        from app.tasks.order_sync import check_api_permissions, diagnose_api_issues
        ...
        logger.exception(f"诊断API设置失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": f"诊断API设置失败: {str(e)}"
        }), 500
...
def test_master_api():
    ...
    """
    try:
        from app.tasks.order_sync import test_master_api_access
        ...
        # 执行测试
        result = test_master_api_access(email, symbol)
